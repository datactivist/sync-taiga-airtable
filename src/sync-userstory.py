...
AIRTABLE_BASE_ID = config["AIRTABLE_BASE_ID_USERSTORY"]
AIRTABLE_TABLE_NAME = config["AIRTABLE_TABLE_NAME_USERSTORY"]
...
# CSV Export URL
CSV_EXPORT_URL = config["CSV_EXPORT_URL_USERSTORY"]

# Headers for authorization
headers = {"Authorization": f"Bearer {AIRTABLE_ACCESS_TOKEN}"}

PIVOT_COLUMN = config["PIVOT_COLUMN_USERSTORY"]
COLUMNS_TO_CHECK = config["COLUMNS_TO_CHECK_USERSTORY"].split(",")
...
if __name__ == "__main__":
    ...
    taiga_df["finish_date"] = (
        pd.to_datetime(taiga_df["finish_date"])
        .dt.strftime("%Y-%m-%dT%H:%M:%S.%f")
        .str[:-3]
        + "Z"
    )
